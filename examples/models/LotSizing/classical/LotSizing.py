...
DEFAULT_DATA = Path(__file__).resolve().parents[2] / "data" / "prob058" / "pigment15d.json"

nPeriods, inventoryCost, changeCosts, nbOfOrders, orders = data or load_json_data(str(DEFAULT_DATA))
...
minimize(
    ...
    Sum(cc) + Sum(ip[i] * inventoryCosts[itemTypes[i]] for i in range(nOrders))
)
