...
minDurations = [[min(durations[s][o] for o in options[t]) for t in T] for s in S]
maxDurations = [[max(durations[s][o] for o in options[t]) for t in T] for s in S]
...
satisfy(
    ...
    # exactly one option selected for each task
    [
        Sum(b[o] for o in options[t]) == 1
        for t in T
    ],

    # duration of each task under each scenario is driven by selected option
    [
        d[s][t] == Sum(durations[s][o] * b[o] for o in options[t])
